...
// --- Utility: strip HTML tags to prevent XSS in emails ---
...
// Middleware
app.use(cors({
  origin: process.env.SITE_URL || 'https://valt.co.za',
  methods: ['GET', 'POST'],
}));
...
app.post('/api/contact', apiLimiter, async (req, res) => {
  try {
    ...
    await transporter.sendMail({
      from: `"VALT Academy Website" <${process.env.SMTP_FROM_EMAIL}>`,
    ...
    await transporter.sendMail({
      from: `"VALT Academy" <${process.env.SMTP_FROM_EMAIL}>`,
      to: req.body.email,
...
// POST /api/enquiry - Programme enquiry form
app.post('/api/enquiry', apiLimiter, async (req, res) => {
  try {
    ...
    await transporter.sendMail({
      from: `"VALT Academy" <${process.env.SMTP_FROM_EMAIL}>`,
      to: req.body.email,
      subject: 'VALT Academy — Enquiry Received',
      html: `
        <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;">
          <div style="background:#0a2342;padding:30px;text-align:center;">
            <h1 style="color:#fff;margin:0;">VALT Academy</h1>
    ...
    res.json({ success: true, message: 'Enquiry submitted! Check your email for a confirmation.' });
  } catch (err) {
